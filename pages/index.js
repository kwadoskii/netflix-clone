...
export default function Home() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `linear-gradient(rgb(0 0 0 / 90%), rgba(0,0,0, 0.5)), url(images/hero.jpg)`,
      }}
    >
      <Head>
      ...
      <header className="flex justify-between w-full px-6 py-5 md:px-12 md:py-8 ">
        <div>
          <svg
        ...
        <div>
          {!session ? (
            <button
              ...
              className="flex items-center bg-[#e50914] text-white px-8 py-2 rounded"
            >
              Sign in
            </button>
          ) : (
      ...
      <main className="flex flex-col items-center justify-center flex-1 ">
        <div className="w-full px-4 text-center py-44 md:px-20">
          <h1 className="max-w-2xl text-6xl font-bold tracking-wide text-white">
            Unlimited movies, TV shows, and more.
          </h1>
          <h2 className="my-4 mb-8 text-2xl text-white">Watch anywhere. Cancel anytime.</h2>
          <p className="text-lg text-white">
            Ready to watch? Enter your email to create or restart your membership.
          </p>

          <div className="flex flex-col mt-4 gap-y-1 md:flex-row">
            <input
              type="text"
              placeholder="Email address"
              className="min-w-[400px] p-4 bg-white outline-none"
            />
            <button
              className="flex items-center self-start gap-3 px-5 py-3.5 text-2xl text-white bg-[#e50914]"
              onClick={() => (session ? console.log("login") : signIn("google"))}
            >
              Get Started
              <svg
                xmlns="http://www.w3.org/2000/svg"
